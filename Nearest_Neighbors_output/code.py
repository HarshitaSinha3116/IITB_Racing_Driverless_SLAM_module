...
pkg_path = get_package_share_directory('nearest_neighbors')
csv_path = os.path.join(pkg_path, 'small_track.csv')
...
class Solution(Bot):
    ...
    def data_association(self, measurements, current_map):
        ...
        if len(measurements) == 0:
          self._global_meas = np.zeros((0, 2))
          self._assoc = np.array([], dtype=int)  # ← close the bracket + dtype
          return
        ...
        for i in range(len(global_meas)):
          point = global_meas[i]
          min_dist = float('inf')
          min_index = -1
...
def compute_nearest_neighbors(perceived_cones, real_cones, threshold):
    """
    Returns an array of indices:
    - index of nearest real cone
    - OR -11 if no valid match
    """
...
def main():
      print("\nComputing nearest neighbor matches...")

      matches = compute_nearest_neighbors(
        BLUE_CONES,     
        YELLOW_CONES,  
        threshold=5
    )
