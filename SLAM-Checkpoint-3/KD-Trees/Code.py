...
class Solution(Bot):
    ...
    # ------------------------------------------------------------------
    def data_association(self, measurements, current_map, threshold=None):
        """
        KD-Tree based Nearest-Neighbour data association.
        Steps:
          1. Transform local measurements → world frame using current pose.
          2. Build a KD-Tree from current_map and query nearest neighbours.
        Returns an int array of indices into current_map (-1 if beyond threshold).
        """
        ...
        global_meas = local_to_global(measurements, self.pos, self.heading)
        assoc = KDTree(global_meas, current_map, threshold)

        self._global_meas = global_meas
        self._assoc = assoc
        return assoc
...
def make_problem1():
    ...
    fig.suptitle("Problem 1 – Data Association  (KD-Trees)",
...
def KDTree(perceived_cones, real_cones, threshold=None):
    tree = cKDTree(real_cones)
    dists, indices = tree.query(perceived_cones, k=1)

    if threshold is not None:
        indices = np.where(dists <= threshold, indices, -1)

    return indices

def main():
    print("\nComputing KD-Tree NN matches...")

    kdtree_matches = KDTree(
        BLUE_CONES,
        YELLOW_CONES,
        threshold=5
    )

    print("KD-Tree Indices:")
    print(kdtree_matches)
