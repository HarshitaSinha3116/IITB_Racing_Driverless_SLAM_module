...
class Solution(Bot):
    ...
    # ------------------------------------------------------------------
    def data_association(self, measurements, current_map,
                      cov=None, R=None, gate_threshold=9.21):
      ...
      assoc = MaxLikelihoodAndIndividualComp(global_meas, current_map, cov, R, gate_threshold)
...
def make_problem1():
    ...
    fig.suptitle("Problem 1 – Data Association  (ML+IC)",
...
def MaxLikelihoodAndIndividualComp(perceived_cones, real_cones,cov=None, R=None, gate_threshold=9.21):
    if cov is None:
        cov = np.eye(2) * 0.5
    if R is None:
    ...
    for p in perceived_cones:
        ...
        for j, r in enumerate(real_cones):
            ...
            lik = np.exp(-0.5 * d2) / (2.0 * np.pi * np.sqrt(det_S))
...
def main():
    ...
    print("\nComputing ML+IC matches...")

    ml_ic_matches = MaxLikelihoodAndIndividualComp(
        BLUE_CONES,
        YELLOW_CONES,
        gate_threshold=9.21
    )
